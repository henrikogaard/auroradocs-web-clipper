...
function setStatus(message, kind = 'info') { busyMessage = null; form.status.replaceChildren(message); form.status.dataset.kind = kind }
function setBusyStatus(message, kind = 'info') { busyMessage = message; form.status.replaceChildren(spinner(), message); form.status.dataset.kind = kind }
...
function setBusy(value) { busy = value; if (!value) busyMessage = null; render() }
function spinner() { const el = document.createElement('span'); el.className = 'spinner'; el.setAttribute('aria-hidden', 'true'); return el }
function avatarInitials(user) { const name = user?.displayName || user?.name || user?.email || ''; const match = name.match(/[A-Za-z0-9]/); return match ? match[0].toUpperCase() : 'A' }
...
function render() {
  ...
  if (form.sessionAvatar) form.sessionAvatar.textContent = avatarInitials(currentSession?.user)
  if (busy && busyMessage) { form.status.replaceChildren(spinner(), busyMessage) }
...
async function readSettings(requireWorkspace = false) {
  ...
  form.apiUrl.value = settings.apiUrl; form.workspaceId.value = settings.workspaceId
  const permissionGranted = await ensureHostPermission(settings.apiUrl, chrome.permissions)
  if (!permissionGranted) throw new Error(`Permission denied for ${settings.apiUrl}.`)
...
async function handleSignIn() {
  try { setBusy(true); setBusyStatus('Signing in...'); const settings = await readSettings(); const email = form.email.value.trim(); const password = form.password.value; if (!email || !password) throw new Error('Email and password are required.'); const result = await signIn({ apiUrl: settings.apiUrl, email, password }); form.password.value = ''; if (result.status === 'mfa_required') { pendingMfa = result.challenge; render(); form.mfaCode.focus(); return } await saveSession(result.session); setStatus('Signed in to AuroraDocs.', 'success') } catch (error) { setStatus(error.message || String(error), 'error') } finally { setBusy(false) }
}

async function handleMfaEmail() { if (!pendingMfa?.mfaId) return; try { setBusy(true); setBusyStatus('Sending email code...'); const settings = await readSettings(); pendingMfa = { ...pendingMfa, ...await requestEmailCode({ apiUrl: settings.apiUrl, email: pendingMfa.email || form.email.value.trim(), mfaId: pendingMfa.mfaId }) }; setStatus('Email code sent.', 'success') } catch (error) { setStatus(error.message || String(error), 'error') } finally { setBusy(false) } }
async function handleMfaVerify() { if (!pendingMfa?.mfaId) return; try { setBusy(true); setBusyStatus('Verifying code...'); const settings = await readSettings(); const method = form.mfaMethod.value || pendingMfa.preferredMethod; if (method === 'email_otp' && !pendingMfa.otpId) pendingMfa = { ...pendingMfa, ...await requestEmailCode({ apiUrl: settings.apiUrl, email: pendingMfa.email || form.email.value.trim(), mfaId: pendingMfa.mfaId }) }; const session = await verifyMfa({ apiUrl: settings.apiUrl, mfaId: pendingMfa.mfaId, otpId: pendingMfa.otpId, method, code: form.mfaCode.value.trim() }); pendingMfa = null; form.mfaCode.value = ''; await saveSession(session); setStatus('Signed in to AuroraDocs.', 'success') } catch (error) { setStatus(error.message || String(error), 'error') } finally { setBusy(false) } }
